...
fnames = os.popen('gsutil ls gs://nmfs_odp_nefsc/NEFSC_CTD_Program_near_real_time_data/TestData/HB2506/*.cnv').read().split('\n')[:-1]
#fnames=fnames.list
...
for x in range(len(fnames)): 
    ...
    cast=[]
    if fnames[x].split('/')[6] == 'ctd999.cnv':
        pass
    else: 
        file = data_dir_fold+'\\'+fnames[x].split('/')[6]
        if os.path.isfile(file):
            ...
            if 't090C' in cast.columns: #check if sbe or ctd profile 
                
                #cast.t090C
    
                cast = cast.rename(columns={"t090C": "temp"})
                
                supname = 'CTD Cast '+fnames[x].split('/')[6].split('.')[0].split('d')[1]+'\n'+file.attributes['gps_datetime']
                
            elif 'tv290C' in cast.columns:
            #except AttributeError:
                #cast.tv290C
                cast = cast.rename(columns={"tv290C": "temp"})
                supname = 'SBE Cast '+fnames[x].split('/')[6].split('.')[0].split('e')[1]+'\n'+file.attributes['gps_datetime']
            else: 
            ...
            if os.path.exists(plotdir + '\Cast'+supname.splitlines()[0].split()[2]+ supname.splitlines()[0].split()[0]+ '.jpg'): 
                ...
                plt.show()
                try: 
                    fig.savefig(plotdir + '\Cast_'+datetime.strptime(date, "%b %d %Y %H:%M:%S").strftime('%Y_%m_%d.%H%M%S')+ '.jpg',  dpi=400, bbox_inches='tight') #, pad_inches = -4)
                except:
